...
class AwrCloudAPI:
    ...
    def assign_keyword_to_new_group(self, keyword_ids, project_id, group_name):
        ...
        return self.post_project_action("projects",
                                        "assign_keywords_new_group",
                                        data)
    ...
    def assign_keyword_to_existing_group(self,
                                         keyword_ids,
                                         project_id,
                                         group_id):
        data = {
    ...
    def post_project_action(self, page, action, data):
        ...
        if self._dry_run:
            print(f"fake posting on url '{url}' with \
                params '{params}' and data '{data}'")
            return FakeResponse()
    ...
    def get_page(self, page, params={}):
        ...
        response = self._session.get(url,
                                     ...
                                     headers=browser_headers)
        if self._debug:
            print(f"GET on page %s" % url + "?"+"&".join([k+"="+v for k, v in iter(params.items())]) if len(params)!=0 else "")
