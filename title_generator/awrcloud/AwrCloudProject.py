from title_generator.awrcloud import pages_parser
...
class AwrCloudProject:
    ...
    def determine_unused_groups(self, used_groups_names):
        return [group for group_name, group in iter(self._groups.items())
                if group_name not in used_groups_names]

    def delete_groups(self, groups):
        self._awr_cloud_connector.delete_groups(self._project_id,
    ...
    def assign_to_group(self, keywords, group):
        ...
        if group in self._groups:
            ...
            response = self._awr_cloud_connector.assign_keyword_to_existing_group(
                kwIds, self._project_id, groupObj.id)
            print(f"{len(kwObjs)} keyphrases to existing group {groupObj}")
        else:
            response = self._awr_cloud_connector.assign_keyword_to_new_group(
                kwIds, self._project_id, group)
            print(f"{len(kwObjs)} keyphrases assigned to new group '{group}'")
    ...
    def fetch_keywords(self):
        def html_to_keyword_dict(html):
            ...
            return {keyword.name: keyword for keyword in keywords}

        perpage = 1000
        keywords_page = self._awr_cloud_connector.get_keywords_page(
            self._project_id, perpage, 0)
