...
from title_generator.commands.base import cli, get_parameter_value

__author__ = 'lin'


@cli.command(name='compare-awr',
             help='Compare generated keywords with AWR Cloud keyword export')
@click.argument('awr-kw-export-file',
                type=click.Path(exists=True),
                default='awr_kw_export.csv')
...
def compare_awr(ctx, awr_kw_export_file, gen_kw_file):
    ...
    awr_kw_export_file = get_parameter_value(
        ctx, "awr-kw-export-file", awr_kw_export_file)
    ...
    awr_keywords = csv.read_csv_column_distinct(
        awr_kw_export_file, 2, quote_char='"')
